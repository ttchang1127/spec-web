...
- **機器欄語彙**（`試驗頻率卡`機器欄，`calcN` 解析）：
  - 桃園（線性/分段）：`per_m3`／`per_m2`／`per_m2_per_layer`／`per_day_min`／`per_batch`／`per_truck`／`per_source`／`per_pour`／`per_*_after_first_100`。
  - 公路局（門檻/計數，2026-07-25 新增）：`per_unit`（每 N 單位取樣）＋`exempt_below`（<X 免驗憑證明）＋`samples_each`（每批抽 K 組）＋`remainder_min`（餘數 ≥R 另計 1 批、<R 併前批）＋`per_ratio`（原廠取樣 %，標註）。
  - 計算：`qty<exempt_below → 0`；否則 `批數＝floor(qty/per_unit)＋(餘數≥remainder_min?1:0)`（至少 1 批），`組數＝批數×samples_each`。示範卡見 KB `07_試驗與抽驗/試驗頻率_防眩板柱`（02861，四欄全用）、`試驗頻率_隔音牆`（02863，純 `per_unit`）。
